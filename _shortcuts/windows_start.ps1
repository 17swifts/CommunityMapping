...
try {
	...
	do {
		Start-Sleep 1;
		$serverStatus = docker inspect --format='{{.State.Health.Status}}' cis_server_1
		...
		$clientStatus = docker inspect --format='{{.State.Health.Status}}' cis_client_1;
	...
	} Until((docker-compose --project-name "cis" ps | Select-String 'Up \(healthy\)' | Measure-Object -Line).Lines -gt 1)
