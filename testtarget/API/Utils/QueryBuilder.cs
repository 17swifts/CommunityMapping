...
namespace APITests.Utils
{
	public static class QueryBuilder
	{
		...
		{
			var attributes = baseEntity.GetEntityAttributeProperties().Select(x => x.Name.LowerCaseFirst()).ToList();
		...
		public static RestSharp.JsonObject BatchUpdateEntityQueryBuilder(List<BaseEntity> baseEntities)
		{
			...
			var attributeNames = baseEntities[0].GetEntityAttributeProperties().Select(x => x.Name).ToList();
