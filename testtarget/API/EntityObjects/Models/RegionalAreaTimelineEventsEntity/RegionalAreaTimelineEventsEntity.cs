...
{
	public class RegionalAreaTimelineEventsEntity : BaseEntity
	{
		// The action taken
		[EntityAttribute]
		...
		/// </summary>
		/// <see cref="Cis.Models.Entity"/>
		public Guid? EntityId { get; set; }


		public RegionalAreaTimelineEventsEntity()
		{
			EntityName = "RegionalAreaTimelineEventsEntity";
			InitialiseReferences();
		}

		public RegionalAreaTimelineEventsEntity(ConfigureOptions option)
		{
			Configure(option);
			InitialiseReferences();
		}
		...
		public override RestSharp.JsonObject ToJson()
		{
			...
			if(Action != null) 
			{
				entityVar["action"] = Action.ToString();
			}
			if(ActionTitle != null) 
			{
				entityVar["actionTitle"] = ActionTitle.ToString();
			}
			if(Description != null) 
			{
			...
			if(GroupId != null) 
			{
				entityVar["groupId"] = GroupId.ToString();
			}
			if (EntityId  != default)
			{
				entityVar["entityId"] = EntityId.ToString();
		...
		public static RegionalAreaTimelineEventsEntity GetEntity(bool isValid, string fixedValue = null)
		{
			if (isValid && !string.IsNullOrEmpty(fixedValue))
			{
				return GetValidEntity(fixedValue);
			}
			return isValid ? GetValidEntity() : GetInvalidEntity<RegionalAreaTimelineEventsEntity>().entity;
		}
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static RegionalAreaTimelineEventsEntity GetValidEntity(string fixedStrValue = null)
		{
			var regionalAreaTimelineEventsEntity = new RegionalAreaTimelineEventsEntity
			{
			};
			regionalAreaTimelineEventsEntity.PopulateAttributes();
