...
namespace ServersideTests.Tests.Integration.BotWritten
{
	...
	public class GroupingServiceTests
	{
		...
		public async Task<IOrderedEnumerable<TimelineGroupDateQueryResult<RegionalAreaTimelineEventsEntity>>> FormClusters(DateRange testRange)
		{
			// Arrange
			...
			var eventEntities = new EntityFactory<RegionalAreaTimelineEventsEntity>(100)
				.UseAttributes()
				.UseReferences()
			...
			return await groupingService.GraphQueryGroupingResult<RegionalAreaTimelineEventsEntity>(timelineFilter);
		}
		...
		public async Task TestNumberOfResultsInCluster(DateRange testRange)
		{
			...
			foreach (var cluster in data)
			{
				...
				var numResultsInDatabase = _database.RegionalAreaTimelineEventsEntity.Count(x => x.Created >= startDate && x.Created < endDate);
