...
namespace ServersideTests.Tests.Integration.BotWritten
{
	public class TwoFactorTests : IDisposable
	{
		...
		public static TheoryData<User> UserTheoryData() {
			return new()
			{
				CreateUserData<AdminEntity>(),
				CreateUserData<ServiceCommissioningBodyEntity>(),
			};
		}
		...
		public static TheoryData<User, string> UserMethodTheoryData()
		{
			return new()
			{
				{ CreateUserData<AdminEntity>(), TokenOptions.DefaultEmailProvider },
				{ CreateUserData<AdminEntity>(), TokenOptions.DefaultAuthenticatorProvider },
				{ CreateUserData<ServiceCommissioningBodyEntity>(), TokenOptions.DefaultEmailProvider },
				{ CreateUserData<ServiceCommissioningBodyEntity>(), TokenOptions.DefaultAuthenticatorProvider },
			};
		}
			...
			where T : User, new()
		{
			...
			using (var scope = _scopeFactory.CreateScope())
			{
				var database = scope.ServiceProvider.GetRequiredService<CisDBContext>();
