...
namespace ServersideTests.Tests.Integration.BotWritten
{
	...
	public class TimelineBehaviourTests : IDisposable
	{
		...
		[Fact]
		public async void RegionalAreaEntityControllerEventLogTest()
		{
			// % protected region % [Configure controller EventLog test for Regional area here] off begin

			// Arrange
			using var entityController = _serviceProvider.GetRequiredService<RegionalAreaEntityController>();
			using var eventController = _serviceProvider.GetRequiredService<RegionalAreaTimelineEventsEntityController>();
			var entityDto = new EntityFactory<RegionalAreaEntity>(1)
				.UseAttributes()
				.UseReferences()
				.DisableIdGeneration()
				.UseOwner(Guid.NewGuid())
				.Generate()
				.Select(x => new RegionalAreaEntityDto(x))
				.First();
			...
			// % protected region % [Configure controller EventLog test for Regional area here] end
		}

		[Fact]
		public async void RegionalAreaTimelineEventsEntityControllerQuickJumpOptionsTest()
		{
			// % protected region % [Configure controller QuickJumpOptions test for Regional area Timeline Events here] off begin
			// Arrange
			using var eventController = _serviceProvider.GetRequiredService<RegionalAreaTimelineEventsEntityController>();
			var random = new Random();

			var eventEntities = new EntityFactory<RegionalAreaTimelineEventsEntity>(5)
				.UseAttributes()
				.UseReferences()
				.UseOwner(Guid.NewGuid())
				.Generate()
			...
			// % protected region % [Configure controller QuickJumpOptions test for Regional area Timeline Events here] end
		}

		[Fact]
		public async void RegionalAreaTimelineEventsEntityControllerGraphDataTest()
		{
			// % protected region % [Configure controller GraphData test for Regional area Timeline Events here] off begin
			// Arrange
			using var eventController = _serviceProvider.GetRequiredService<RegionalAreaTimelineEventsEntityController>();
			var random = new Random();
			...
			// % protected region % [Configure controller GraphData test for Regional area Timeline Events here] end
		}
