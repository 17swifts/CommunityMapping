...
namespace Cis.Services
{
    public class GapScoreService
    {
        ...
        public GapScoreService(CisDBContext dbContext)
        {
            // Retrieve a dbContext from dependency injection and assign it to a class variable
            _dbContext = dbContext;
        }

        public async Task UpdateGapScoreService(CancellationToken cancellationToken = default)
        {
        ...
        //     var population = regionalArea.Indigenous + regionalArea.Nonindigenous;
        //     var services = regionalArea.Servicess;
        //     var servicesGroupedByCategory = services.Where(service => service.Active == "TURE")
        //         .GroupBy(service => service.Category);
            
        //     foreach(var group in servicesGroupedByCategory){
        //         var currentServices = group.Count();
        //         var servicesNeeded = 0;
        //         switch(group.Key)
        //         {
        //         case Categories.ABORIGINAL_SERVICE:
        //             servicesNeeded = (needForAboriginalServiceByPop * regionalArea.Indigenous) / averageNumberPeopleAServiceCanService;
        //             break;
