...
namespace Cis.Services
{
	public class CrudService : ICrudService, IDisposable
	{
			...
			CancellationToken cancellation = default)
		{
			...
			fileQuery = fileQuery.Include(f => f.ServiceCommissioningBodyProfileImage);
			...
			if (!options.DisableSecurity)
			{
				...
				var conditions = new Expression[]
				{
					...
					CreateFileSecurityCondition<ServiceCommissioningBodyEntity>(dbContext, baseParam, "ServiceCommissioningBodyProfileImage"),
				};
